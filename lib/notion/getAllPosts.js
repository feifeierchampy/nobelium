...
export async function fillPostSummaries (posts, length = BLOG.summaryLength || 200) {
  ...
  for (let i = 0; i < posts.length; i++) {
    ...
    if (summary) {
      post.summary = summary.length > length ? summary.slice(0, length).trim() : summary
      continue
    }
...
export async function getAllPosts ({ includePages = false }) {
  ...
  if (
    ...
    for (let i = 0; i < pageIds.length; i++) {
      ...
      properties.summary = summary.length > summaryLength ? summary.slice(0, summaryLength).trim() : summary
